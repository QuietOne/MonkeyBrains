...
/**
 * "Wander is a type of random steering. This idea can be implemented several ways, 
 * but one that has produced good results is to constrain the steering force to the
 * surface of a sphere located slightly ahead of the character. To produce the steering 
 * force for the next frame: a random displacement is added to the previous value,
 * and the sum is constrained again to the sphere's surface. The sphere's radius
 * determines the maximum wandering strength and the magnitude of the random displacement 
 * determines the wander 'rate'." <br><br>
 * 
...
public class SphereWanderBehaviour extends AbstractStrengthSteeringBehaviour {
    ...
    /** 
     * The sphere radius is 0.75 by default. Note that low radius values can cause
     * unexpected errors.
     * 
     * @throws SphereWanderWithoutRadius If sphereRadius is lower or equals to 0
     */
    public void setSphereRadius(float sphereRadius) {
        if(sphereRadius <= 0) throw new SphereWanderWithoutRadius("The sphere radius must be possitive. Current value is: " + sphereRadius);
     ...
     * @param rotationFactor Defines the maximum random variaton for each iteration.
     * 
     * @throws SphereWanderWithoutTimeInterval If timeInterval is lower or equals to 0
     * @throws SphereWanderInvalidRandomFactor If randomFactor is not contained in the [0,1] interval
     * @throws SphereWanderInvalidRotationFactor If rotationFactor is not contained in the [0,1] interval
     */
    ...
    /**
     * @see SphereWanderBehaviour#SphereWanderBehaviour(com.jme3.ai.agents.Agent, float, float, float) 
    ...
    {
        if(timeInterval <= 0)
            throw new SphereWanderWithoutTimeInterval("The time interval must be possitive. The current value is: " + timeInterval);
        else if(randomFactor < 0 || randomFactor > 1)
            throw new SphereWanderInvalidRandomFactor("The random factor value must be contained in the [0,1] interval. The current value is " + randomFactor);
        else if(rotationFactor < 0 || rotationFactor > 1)
            throw new SphereWanderInvalidRotationFactor("The rotation factor value must be contained in the [0,1] interval. The current value is " + rotationFactor);
        
        this.timeInterval = timeInterval;
        this.time = this.timeInterval;
        this.randomFactor = randomFactor;
        
        this.wanderSphere = new BoundingSphere
                (
                    this.sphereRadius,
                    Vector3f.ZERO
                );  
    ...
    {     
        ...
        if(rand.nextBoolean()) extraRandomSide *= -1;
